...
const getProductImage = (product) => product?.url || product?.detailUrl || placeholderProductImage;
                    ...
                    data.map(temp => (
                        <Link to={`product/${temp.id}`} style={{textDecoration: 'none'}}>
                            <Box textAlign="center" style={{ padding: '25px 15px' }}>
                                <Image
                                    src={getProductImage(temp)}
                                    alt={temp.title?.shortTitle || 'product image'}
                                    loading="lazy"
                                    onError={(e) => { e.currentTarget.src = placeholderProductImage; }}
                                />
                                <Text style={{ fontWeight: 600, color: '#212121' }}>{temp.title.shortTitle}</Text>
                                <Text style={{ color: '#AEB784' }}>{temp.discount}</Text>
